...
final class IndexOps[T](val lhs: T) extends AnyVal {
  ...
  def indexToIndexedSeq[@spec(Int) A](implicit ev: Index[A, T]): IndexedSeq[A] = 
    ev.indexToIndexedSeq(lhs)
  def indexElement[@spec(Int) A](rhs: Int)(implicit ev: Index[A, T]): A = 
    ev.indexElement(lhs, rhs)
}

class IndexLexicographicEq[@spec(Int) A, T](implicit val index: Index[A, T], implicit val eq: Eq[A])  extends Eq[T] {
...
class IndexLexicographicOrder[@spec(Int) A, T](implicit val index: Index[A, T], implicit val order: Order[A]) {
...
class ArrayIndex[@spec(Int) A] extends Index[A, Array[A]] {
  def indexElement(t: Array[A], i: Int) = t(i)
  def indexLength(t: Array[A]) = t.length
}

class IndexedSeqIndex[@spec(Int) A] extends Index[A, IndexedSeq[A]] {
