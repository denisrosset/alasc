...
case class TransversalExplicit[F <: Finite[F]](beta: Dom, action: Action[F], intMap: IntMap[TEntry[F]]) extends Transversal[F] with TransversalImpl[F] {
  ...
  def mapValues[G <: Finite[G]](f: F => G, gAction: Action[G]): TransversalExplicit[G] =
    TransversalExplicit(beta, gAction, IntMap.empty[TEntry[G]] ++ intMap.mapValues( wi => TEntry(f(wi.u), f(wi.uinv), gAction, wi.isIdentity)))
  ...
  def updated(newGens: Iterable[F], allGens: Iterable[F]): TransversalExplicit[F] = {
    ...
    var candidates: IntMap[TEntry[F]] = IntMap.empty[TEntry[F]] ++ (
      ...
      yield (img._0, TEntry(apply(b).u*s, sinv*apply(b).uinv, action)) )
    ...
    def checkCandidate(candidate: (Int, TEntry[F])): Boolean = {
      val (b, TEntry(u, uinv, _, _)) = candidate
      for ((s, sinv) <- allGensInv; img = action(s, b) if !newMap.contains(img)) {
        val el = (img._0, TEntry(u*s, sinv*uinv, action))
  ...
  def conjugatedBy(f: F, finv: F): TransversalExplicit[F] = {
    val newMap: IntMap[TEntry[F]] = intMap.map { case (b, TEntry(v,vinv,action,isIdentity)) => (action(f, b)._0, TEntry(finv*v*f, finv*vinv*f, action, isIdentity)) } // TODO: bring to class itself
    TransversalExplicit(action(f, beta), action, newMap)
  }
}

object TransversalExplicit extends TransversalBuilder {
  def empty[F <: Finite[F]](beta: Dom, action: Action[F]) = TransversalExplicit(beta, action, IntMap(beta._0 -> TEntry(action.identity, action.identity, action, true)))
}
