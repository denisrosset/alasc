...
trait Representations[G] {
  def minimal: Representation[G]
  /** If `r` is a representation of the correct type, returns `r`, or creates a new representation from
    * `generators`.
    */ 
  def get(r: Representation[G], generators: Iterable[G]): Representation[G]
...
/** Implementation of a class of representations with a specific type. */
