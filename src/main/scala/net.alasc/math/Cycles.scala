...
class Cycles private[alasc](val seq: Seq[Cycle]) {
  ...
  def apply(cycle: Int*) = Cycles.permutation.op(this, Cycles(cycle: _*))
}
...
class CyclesPermutation extends Permutation[Cycles] {
  ...
  def op(x: Cycles, y: Cycles) = 
    Cycles.fromSupportAndImages(support(x) ++ support(y), i => actr(actr(i, x), y))
...
trait CyclesInstances {
  final implicit val permutation = new CyclesPermutation
}

object Cycles extends CyclesInstances with PermutationBuilder[Cycles] {
  def empty = new Cycles(Seq.empty[Cycle])
