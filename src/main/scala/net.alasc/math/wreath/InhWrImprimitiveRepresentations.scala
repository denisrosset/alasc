...
abstract class InhWrRepresentations[A, H] extends Representations[Wr[A, H]] {
  ...
  implicit def arClassTag: ClassTag[aReps.R]
  type AR = aReps.R
  case class Shape(partition: Domain#Partition, repForBlock: Array[aReps.R]) {
    def size = partition.size
    def repForIndex(i: Int) = repForBlock(partition.blockIndex(i))
    def forSize(newSize: Int): RefOption[Shape] =
      if (newSize == partition.size)
        ...
        RefSome(Shape(newPartition, newRepForBlock))
  ...
  object Shape {
    implicit object lattice extends BoundedBelowLattice[Shape] {
      def zero = Shape(Domain(1).Partition.fromArray(Array(immutable.BitSet(0))), Array(aReps.lattice.zero))
      override def lteqv(x: Shape, y: Shape): Boolean = // x <= y ?
        if (y.size < x.size) false else {
          val xSized = x.forSize(y.size).get
          val domain = Domain(x.size)
          implicit def partitionLattice: BoundedLattice[domain.Partition] = domain.lattice
      ...
      override def gteqv(x: Shape, y: Shape): Boolean = lteqv(y, x)
      def partialCompare(x: Shape, y: Shape): Double =
        if (lteqv(x, y)) {
          if (gteqv(x, y))
            0.0
      ...
      def join(x: Shape, y: Shape): Shape = {
        ...
        Shape(newPartition, newRepForBlock)
      }
      def meet(x: Shape, y: Shape): Shape = {
        ...
        implicit def workPartitionLattice: BoundedLattice[workDomain.Partition] = workDomain.lattice
        implicit def newPartitionLattice: BoundedLattice[newDomain.Partition] = newDomain.lattice
        implicit def aRepLattice: Lattice[aReps.R] = aReps.lattice
