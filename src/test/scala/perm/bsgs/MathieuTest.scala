...
object M11 {
  ...
  def g = BSGS.schreierSims(List(a,b),Sym(11).identity)
...
object M12 {
  ...
  def g = BSGS.schreierSims(List(a,b,c),Sym(12).identity)
...
object M24 {
  ...
  def g = BSGS.schreierSims(List(a,b,c),Sym(24).identity)
...
class MathieuSuite extends FunSuite {
  test("Construct BSGS for M11 using deterministic/empty base and check order") {
    import M11._
    val bsgs = BSGS.schreierSims(List(a,b), Sym(11).identity, EmptyBase)
  ...
  test("Construct BSGS for M12 using deterministic/empty base and check order") {
    import M12._
    val bsgs = BSGS.schreierSims(List(a,b,c), Sym(12).identity, EmptyBase)
    assert(bsgs.order == order)
  }
