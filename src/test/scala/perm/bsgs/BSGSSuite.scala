...
/*
  test("Example 2.2 of Thomas Rehn diploma thesis") {
    val g12 = Perm(4)(List(1,2).map(Dom._1(_)))
    val g23 = Perm(4)(2,3)
    val g34 = Perm(4)(3,4)
    val g = BSGS.randomSchreierSims(PrescribedBase(List(4,3,2).map(Dom._1(_))), ExplicitTransversalFactory[Perm]())(.construct(List(3,2,1), List(g01,g12,g23), ExplicitTransversal.fromGenerators[ExplicitPermutation])
    assert(G.order == 24)
  }
