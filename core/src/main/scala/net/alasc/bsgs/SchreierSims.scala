...
/** Data structure to store the mutable chain of the kernel of a group.
...
abstract class KernelBuilder[G] {
  ...
  def result(): Chain.Generic[G] = _opt match {
...
object KernelBuilder {
  ...
  private[this] val trivialInstance = new KernelBuilder[AnyRef] {
    ...
    protected def makeMutableChain() = sys.error("This kernel should be trivial")
    override def result(): Chain.Generic[AnyRef] = Term.generic[AnyRef]
  }
...
object SiftResult {
  ...
  /** Represents the result of a completely sifted element not equal to the identity, thus
    * part of the kernel of the action. */
...
object SchreierSims {
  ...
  /** Finds if a new strong generator can be found at the given `node`, assuming that
    ...
    * - SiftResult.NotId: there is a strong generator for the kernel to test
      ...
      node.strongGeneratingSet.foreach { x =>
        ...
        if (ubx =!= node.u(i)) {
          ...
          siftAndUpdateBaseFrom(mutableChain, node, schreierGen) match {
            ...
            case SiftResult.NotId(kg) => // we have an nontrivial element to sift through the kernel
  ...
  /** Deterministic Schreier-Sims algorithm, completes the given mutable chain and the given kernel as well. */
  ...
  /** Deterministic Schreier-Sims algorithm. Constructs a BSGS chain, and adds completely sifted generators to the kernel,
    * if relevant.
    */
