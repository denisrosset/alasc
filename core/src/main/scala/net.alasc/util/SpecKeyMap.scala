 ...
 */
final class SpecKeyMap[@spec(Int, Long) A, B] protected[debox] (ks: Array[A], vs: Array[B], bs: Array[Byte], n: Int, u: Int)(implicit val cta: ClassTag[A], val ctb: ClassTag[B]) { lhs =>
   ...
   * This is an O(n) operation.
   */
  final def copy: SpecKeyMap[A, B] = new SpecKeyMap(keys.clone, vals.clone, buckets.clone, len, used)
   ...
   */
  final def clear: Unit = absorb(SpecKeyMap.empty[A, B])
   ...
   */
  private[this] def absorb(rhs: SpecKeyMap[A, B]): Unit = {
    keys = rhs.keys
    vals = rhs.vals
    buckets = rhs.buckets
    len = rhs.len
   ...
   */
  final def ++=(rhs: SpecKeyMap[A, B])(implicit ev: Monoid[B]): Unit = {
    val z = ev.id
   ...
   * maps.
   */
  final def ++(rhs: SpecKeyMap[A, B])(implicit ev: Monoid[B]): SpecKeyMap[A, B] = {
    val out = lhs.copy
    out ++= rhs
    out
  }
   ...
   * map.
   */
  final def mapToArray[@spec C: ClassTag](f: (A, B) => C): Array[C] = {
   ...
   * map.
   */
  final def mapToSet[@spec C: ClassTag](f: (A, B) => C): Set[C] = {
   ...
   * map.
   */
  final def mapItemsToMapUnsafe[@spec (Int, Long) C: ClassTag, D: ClassTag](f: (A, B) => (C, D)): SpecKeyMap[C, D] = {
    val result = SpecKeyMap.ofSize[C, D](len)
   ...
   * map.
   */
  final def mapItemsToMap[@spec (Int, Long) C: ClassTag, D: ClassTag](f: (A, B) => (C, D))(implicit ev: CMonoid[D]): SpecKeyMap[C, D] = {
    val result = SpecKeyMap.ofSize[C, D](len)
    val z = ev.id
    foreach { (a, b) =>
      val (c, d) = f(a, b)
   ...
   * map.
   */
  final def mapKeysUnsafe[@spec (Int, Long) C: ClassTag](f: A => C): SpecKeyMap[C, B] = {
    val result = SpecKeyMap.ofSize[C, B](len)
    foreach((a, b) => result(f(a)) = b)
    result.compact
    result
  }
   ...
   * map.
   */
  final def mapKeys[@spec (Int, Long) C: ClassTag](f: A => C)(implicit ev: CMonoid[B]): SpecKeyMap[C, B] = {
    val result = SpecKeyMap.ofSize[C, B](len)
    val z = ev.id
    foreach { (a, b) =>
      val c = f(a)
   ...
   * map.
   */
  final def mapValues[C: ClassTag](f: B => C): SpecKeyMap[A, C] = {
    val arr = new Array[C](buckets.length)
    cfor(0)(_ < buckets.length, _ + 1) { i =>
      if (buckets(i) == 3) arr(i) = f(vals(i))
    }
    new SpecKeyMap[A, C](keys.clone, arr, buckets.clone, len, used)
  }
  ...
  final def grow(): Unit2[A, B] = {
    ...
    val map = SpecKeyMap.ofSize[A, B](next)
    cfor(0)(_ < buckets.length, _ + 1) { i =>
  ...
  final def compact(): Unit2[A, B] = {
    val map = SpecKeyMap.ofSize[A, B](len)
    cfor(0)(_ < buckets.length, _ + 1) { i =>
      if (buckets(i) == 3) map(keys(i)) = vals(i)
    }
    absorb(map)
   ...
   * This is an O(n) operation, where n is the size of the map.
   */
  def findAll(p: (A, B) => Boolean): SpecKeyMap[A, B] = {
    val out = SpecKeyMap.empty[A, B]
...
object SpecKeyMap {
   ...
   * Create an empty Map.
   * 
   * Example: SpecKeyMap.empty[Int, String]
   */
  def empty[@spec(Int, Long) A: ClassTag, B: ClassTag]: SpecKeyMap[A, B] =
    new SpecKeyMap(new Array[A](8), new Array[B](8), new Array[Byte](8), 0, 0)
   ...
   * performance.
   * 
   * Example: SpecKeyMap.ofSize[Int, String](100).
   */
  def ofSize[@spec(Int, Long) A: ClassTag, B: ClassTag](n: Int): SpecKeyMap[A, B] =
   ...
   * you want instead.
   */
  private[debox] def ofAllocatedSize[@spec(Int, Long) A: ClassTag, B: ClassTag](n: Int) = {
    val sz = Util.nextPowerOfTwo(n) match {
      case n if n < 0 => throw DeboxOverflowError(n)
      case 0 => 8
      case n => n
    }
    new SpecKeyMap(new Array[A](sz), new Array[B](sz), new Array[Byte](sz), 0, 0)
  }
   ...
   * 
   * Example: SpecKeyMap(1 -> "cat", 2 -> "dog", 3 -> "fish")
   */
  def apply[@spec(Int, Long) A: ClassTag, B: ClassTag](pairs: (A, B)*): SpecKeyMap[A, B] =
    fromIterable(pairs)

  /**
   * Create a map from an array of keys and another array of values.
   * 
   * Example: SpecKeyMap(Array(1,2,3), Array("cat", "dog", "fish"))
   */
  def fromArrays[@spec(Int, Long) A: ClassTag, B: ClassTag](ks: Array[A], vs: Array[B]): SpecKeyMap[A, B] = {
   ...
   * require fewer allocations.
   * 
   * Example: SpecKeyMap.fromIterable(List((1, "cat"), (2, "dog"), (3, "fish")))
   */
  def fromIterable[@spec(Int, Long) A: ClassTag, B: ClassTag](pairs: Iterable[(A, B)]): SpecKeyMap[A, B] = {
  ...
  /**
   * Provide an Eq[SpecKeyMap[A, B]] instance.
   * 
  ...
  implicit def eqv[A, B] =
    new Eq[SpecKeyMap[A, B]] {
      def eqv(lhs: SpecKeyMap[A, B], rhs: SpecKeyMap[A, B]): Boolean = lhs == rhs
    }

  /**
   * Provide a Monoid[SpecKeyMap[A, B]].
   * 
   * The maps are combined key-by-key, using |+| to merge values if
   * necessary.
   */
  implicit def monoid[@spec(Int, Long) A: ClassTag, B: ClassTag: Monoid] =
    new Monoid[SpecKeyMap[A, B]] {
      def id: SpecKeyMap[A, B] = SpecKeyMap.empty[A, B]
      def op(lhs: SpecKeyMap[A, B], rhs: SpecKeyMap[A, B]): SpecKeyMap[A, B] = lhs ++ rhs
    }

  /**
   * Provide an AdditiveMonoid[SpecKeyMap[A, B]].
   * 
   * The maps are combined key-by-key, using + to merge values if
   * necessary.
   */
  implicit def additiveMonoid[@spec(Int, Long) A: ClassTag, B: ClassTag: AdditiveMonoid] =
    new AdditiveMonoid[SpecKeyMap[A, B]] {
      implicit val m: Monoid[B] = implicitly[AdditiveMonoid[B]].additive
      def zero: SpecKeyMap[A, B] = SpecKeyMap.empty[A, B]
      def plus(lhs: SpecKeyMap[A, B], rhs: SpecKeyMap[A, B]): SpecKeyMap[A, B] = lhs ++ rhs
    }

  /**
   * Provide an MultiplicativeMonoid[SpecKeyMap[A, B]].
   * 
   * The maps are combined key-by-key, using * to merge values if
   * necessary.
   */
  implicit def multiplicativeMonoid[@spec(Int, Long) A: ClassTag, B: ClassTag: MultiplicativeMonoid] =
    new MultiplicativeMonoid[SpecKeyMap[A, B]] {
      implicit val m: Monoid[B] = implicitly[MultiplicativeMonoid[B]].multiplicative
      def one: SpecKeyMap[A, B] = SpecKeyMap.empty[A, B]
      def times(lhs: SpecKeyMap[A, B], rhs: SpecKeyMap[A, B]): SpecKeyMap[A, B] = lhs ++ rhs
    }
}
