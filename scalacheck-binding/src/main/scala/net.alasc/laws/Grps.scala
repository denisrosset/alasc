...
object Grps {
  ...
  def fromElements[G:Eq:Group:Representations:ClassTag](elements: Gen[G]): Gen[Grp[G]] =
    for {
      n <- Gen.choose(2, 4)
      generators <- Gen.containerOfN[Seq, G](n, elements)
    } yield Grp(generators: _*)

  implicit def arbGrp[G:Arbitrary:Eq:Group:Representations:ClassTag]: Arbitrary[Grp[G]] = 
    Arbitrary {
  ...
  implicit def instances[G:Instances:Eq:Group:Representations:ClassTag]: Instances[Grp[G]] =
